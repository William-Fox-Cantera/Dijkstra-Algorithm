...
void Graph::dijkstra() { // 5 pts
	...
	for (int i = 0; i < numOfVerts; i++) {
		distances[i] = adjMatrix[start][i]; // rc, rows columns
	}

	// Step 3
	for (int count = 0; count < numOfVerts; count++) {
		printInfoSoFar();
		int latestVert = minDistance();
		setDistances(latestVert);
	}
}
...
void Graph::setDistances(int latestVert) { //8 pts
	for (int i = 0; i < numOfVerts; i++) {
		if ((!visited[i]) && ((distances[latestVert] + adjMatrix[latestVert][i]) < distances[i])) {
...
//This method finds the next unvisited vertex with the minimum distance.
